...
public class BookManag {
    ...
    private static int memberselect=2;   // member number
        ...
        while (user_menu) {
            ...
            if (selectmem > last_num || selectmem < 0) {
                ...
                while(!exit) {
                    // 메뉴 출력
                    System.out.println("----------------------------------------");
                    System.out.println("   1. 대출     2. 반납     3. 대여 현황  4. 종료       ");
                    System.out.println("----------------------------------------");
                        ...
                        if(booksForReturn) { // 반납할 도서가 있다면
    	                    ...
    	                    for(int i = 0; i < borrowList.size(); i++) {
	                        	if ((borrowList.get(i).getMemberId() == bm.memberselect)) {
	                                System.out.printf("%2d %-35s %-35s %-35s %-20s -20s\n", borrowList.get(i).getBookId(), bookList.get(i).getTitle(), bookList.get(i).getAuthor(), bookList.get(i).getPublisher(), sdf.format(borrowList.get(i).getStartDay()), borrowList.get(i).getReturnDay());
	                            }
	                        }
