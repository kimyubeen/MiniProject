...
//                               }
                            ...
                            for (Borrow brw : borrowList) {
                                if(b.getBookid() == booknum) {
                                    if(borrowList.get(booknum - 1).isStatus() == false) {
                                        ...
                                        borrowList.get(booknum - 1).setMemberId(3); //임시사용자
                                        //borrowList.get(booknum - 1).setMemberId(bm.memberselect); //로그인 부분이랑 연결해서 생각하기!!!
                                        // 대여가능 기간 출력
                        ...
                        System.out.println("----------------------------------------------------------------------------------------------------------------------------");
                        System.out.printf("%-20s %-35s %-35s %-35s %-20s %-20s\n", "책번호", "제목", "작가", "출판사", "대출일", "반납일");
                        System.out.println("----------------------------------------------------------------------------------------------------------------------------");
                        for (Book b : bookList) {
                            for (Borrow brw : borrowList) { //중복되는 값 출력되는 거 수정하기!!!!
                                if (brw.getMemberId() == 2) {
                                    System.out.printf("%2d %-35s %-35s %-35s %2d %-20s\n", brw.getBookId(), b.getTitle(), b.getAuthor(), b.getPublisher(), brw.getBookId(), brw.getStartDay(), brw.getReturnDay());
                                    break;
                                }
                            }
                        }
